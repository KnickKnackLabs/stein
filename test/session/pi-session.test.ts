...
describe("Pi session factory", () => {
  ...
  test("rejects blank active and custom tool names before SDK use", () => {
    expect(() => createPiSessionFactory({ ...config, tools: [" "] })).toThrow("tools[0]");
    const customTool = createReadToolDefinition(config.workspaceDirectory);
    customTool.name = "";
    expect(() => createPiSessionFactory({ ...config, customTools: [customTool] })).toThrow(
      "customTools[0].name",
    );
  });
  ...
  test("rejects empty model and prompt configuration before SDK use", () => {
    expect(() =>
      createPiSessionFactory({ ...config, model: { provider: "", id: "deterministic" } }),
  ...
  test("rejects relative runtime paths before SDK use", () => {
    expect(() => createPiSessionFactory({ ...config, sessionDirectory: "sessions" })).toThrow(
      "sessionDirectory must be an absolute path",
    );
  });
