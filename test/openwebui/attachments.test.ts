...
import { CHAT_COMPLETION_LIMITS, parseChatCompletion } from "../../src/openai/chat-completion.ts";
...
function parse(content: string, attachments?: readonly unknown[]) {
  return parseChatCompletion(
    ...
    MODEL,
    kklOpenWebUiAttachmentNormalizer,
  );
}
...
describe("KKL Open WebUI attachments", () => {
  ...
  test("rejects invalid embedded names and enforces the shared attachment limit", () => {
    expect(() => parse('<attached_file name="">text</attached_file>')).toThrow(
      "embedded attachment must contain a name",
    );
